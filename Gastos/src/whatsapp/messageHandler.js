...
async function handleGastosMessage(msg, sock, spreadsheetId) {
  ...
  try {
    ...
    // --- PDF mensual: comando explícito o lenguaje natural ---
    const _wantsPdf = textLower === '/pdf' || textLower === '/reporte' || textLower === '/informe' ||
      /\b(dame|env[ií]ame|genera|crea|quiero|manda|hazme)\s+(el\s+)?(informe|reporte|pdf|resumen\s+pdf)\s*(mensual|de\s+gastos|del\s+mes)?\b/i.test(textLower) ||
      /\b(informe|reporte)\s+(mensual|de\s+gastos|del\s+mes)\b/i.test(textLower);

    if (_wantsPdf) {
      await _reply(sock, jid, msg, '⏳ Generando reporte PDF del mes, un momento...');
      try {
